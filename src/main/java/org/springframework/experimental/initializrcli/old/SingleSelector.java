...
public class SingleSelector {
	...
	public SingleSelector(Terminal terminal, String title, Collection<String> options) {
	...
	private List<AttributedString> displayLines(int cursorRow) {
		...
		for (String s : lines) {
			...
			if (i == cursorRow) {
				builder.append("> " + s, AttributedStyle.DEFAULT.foreground(AttributedStyle.BRIGHT + AttributedStyle.CYAN).bold());
			} else {
				builder.append("  " + s);
			}
			i++;
